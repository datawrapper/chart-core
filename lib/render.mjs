...
function renderChart() {
    ...
    // if the chart object isn't initialized yet, we can't render
    if (!chart) return;
    ...
    addClass(document.body, `vis-height-${heightMode}`);
    removeClass(
        document.querySelector('.dw-chart'),
        `vis-height-${heightMode === 'fit' ? 'fixed' : 'fit'}`
    );
    addClass(document.querySelector('.dw-chart'), `vis-height-${heightMode}`);
