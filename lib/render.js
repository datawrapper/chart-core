...
function getHeightMode() {
    ...
    const isFitChart = ids => get(theme, `vis.${ids[0]}.fitchart`, false) && ids.includes(vis.id);

    const themeFitChart = [pieFamily, barFamily, tableFamily].filter(isFitChart).length;
