...
function getHeightMode() {
    ...
    const pieFamily = ['d3-pies', 'd3-donuts', 'd3-multiple-pies', 'd3-multiple-donuts'];
    const barFamily = [
        'd3-bars',
        'd3-bars-stacked',
        'd3-bars-split',
    ...
    const tableFamily = ['tables'];

    const checkForFitChart = types =>
        get(theme, `vis.${types[0]}.fitchart`, false) && types.includes(vis.id);

    const themeFitChart = [pieFamily, barFamily, tableFamily].filter(checkForFitChart).length;
