...
async function chartLoaded() {
    const externalJSON = get(__dw.params.chartJSON, 'metadata.data.use-datawrapper-cdn')
        ? `//${__dw.params.publishData.externalDataUrl}/${__dw.params.chartJSON.id}.metadata.json`
        : get(__dw.params.chartJSON, 'metadata.data.external-json');

    if (!__dw.params.preview && externalJSON) {
