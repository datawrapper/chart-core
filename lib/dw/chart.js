...
export default function(attributes) {
    ...
    const chart = {
        ...
        render(container, isIframe, isPreview) {
            if (!visualization) {
            ...
            isIframe && initResizeHandler(container, vis);
            isIframe && postMessage(container, chart, vis, isPreview);

            function initResizeHandler(container, vis) {
                const resize = chart.getHeightMode() === 'fixed' ? resizeFixed : renderLater;
                ...
                function renderLater() {
                    ...
                    reloadTimer = setTimeout(() => {
                        renderChart(container, styleHolder, chart, vis, isIframe, isPreview);
                    }, 100);
                }
            }

            function postMessage(container, chart, vis, isPreview) {
                let desiredHeight;
