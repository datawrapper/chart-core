...
export default function(attributes) {
    ...
    const chart = {
        ...
        load(csv, externalData, externalJSON) {
            const dsopts = {
                firstRowIsHeader: chart.getMetadata('data.horizontal-header', true),
                transpose: chart.getMetadata('data.transpose', false)
            };

            if ((csv || csv === '') && !externalData) dsopts.csv = csv;
            else dsopts.url = externalData || 'data.csv';

            const datasource = chart.getMetadata('data.json') ? json(dsopts) : delimited(dsopts);

            return datasource
                .dataset()
                .then(ds => {
                    if (chart.getMetadata('data.external-json')) {
                        return window
                            .fetch(chart.getMetadata('data.external-json'))
                            .then(res => res.json())
                            .then(obj => {
                                Object.assign(chart.get().metadata, obj);
