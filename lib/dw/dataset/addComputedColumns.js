...
export default function addComputedColumns(chart, dataset) {
    ...
    const vNamesToVar = Object.keys(virtualColumns).reduce((acc, val, idx) => {
        const key = columnNameToVariable(val);
        return acc.set(key, {
            name: val,
            origIndex: idx,
            key,
            formula: virtualColumns[val],
            visited: 0,
            computed: false,
            dependsOn: []
        });
    ...
    Object.entries(virtualColumns).forEach(([name, formula]) => {
    ...
    computedColumns
        ...
        .forEach(({ column }) => dataset.add(column));
